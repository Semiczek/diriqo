...
-- Public quote sharing metadata used by the quote detail backfill and public offer links.
alter table public.quotes
  add column if not exists share_token_scope text default 'quote_public_offer',
...
where share_token_scope is null
  or btrim(share_token_scope) = '';

alter table public.quotes
  alter column share_token_scope set default 'quote_public_offer',
  alter column share_token_scope set not null;

create unique index if not exists quotes_share_token_uidx
  on public.quotes (share_token)
  where share_token is not null;

create index if not exists quotes_public_offer_token_status_idx
  on public.quotes (share_token, share_token_scope, share_token_expires_at)
  where share_token is not null
    and share_token_revoked_at is null;

-- Safe job delete RPC. The client already calls this RPC; keep the write guard in the database.
