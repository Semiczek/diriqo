...
-- Adds safe admin repair fields for job_assignments.
-- Archived assignments keep audit history but are excluded from labor/economics views.
...
alter table public.job_assignments
  ...
  add column if not exists reset_by uuid null references public.profiles(id),
  add column if not exists reset_reason text null;

create index if not exists job_assignments_active_job_profile_idx
on public.job_assignments (job_id, profile_id)
