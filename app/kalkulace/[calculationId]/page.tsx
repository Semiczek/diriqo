...
export default async function GlobalCalculationDetailPage({ params }: PageProps) {
  ...
  return (
    <DashboardShell activeItem="kalkulace">
      ...
      >
        ...
        >
          ...
          >
            <div>
              ...
              <div style={heroTextStyle}>{customer?.name || 'Bez zákazníka'}</div>
            </div>
            ...
            >
              ...
              <Link
                href={`/kalkulace/${calculation.id}/edit`}
                style={{
                  display: 'inline-flex',
                  alignItems: 'center',
                  padding: '10px 14px',
          ...
          >
            <div style={customerPanelStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '14px' }}>
                <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                  <div style={{ color: '#166534', marginBottom: '6px' }}>Cena pro zákazníka</div>
                  <strong>{formatCurrency(calculation.total_price ?? calculation.subtotal_price)}</strong>
                </div>
                <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                  <div style={{ color: '#166534', marginBottom: '6px' }}>Předpokládaný zisk</div>
            ...
            <div style={costPanelStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '14px' }}>
                <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                  <div style={{ color: '#c2410c', marginBottom: '6px' }}>Datum</div>
                  <strong>{formatDate(calculation.calculation_date)}</strong>
                </div>
                <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                  <div style={{ color: '#c2410c', marginBottom: '6px' }}>Náklady</div>
          ...
          <div style={{ display: 'grid', gap: '10px' }}>
            ...
            <div>
              <strong>Interní poznámka:</strong> {calculation.internal_note || '-'}
            </div>
          </div>
          ...
          <div style={{ ...summaryPanelStyle, marginTop: '16px' }}>
            ...
            >
              <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                <div style={{ color: '#1d4ed8', marginBottom: '6px' }}>Zisk</div>
                <strong>{formatCurrency(profit)}</strong>
              </div>
              <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                <div style={{ color: '#1d4ed8', marginBottom: '6px' }}>Marže</div>
                <strong>{marginPercent.toFixed(1)} %</strong>
              </div>
              <div style={{ padding: '14px', borderRadius: '12px', backgroundColor: '#ffffff' }}>
                <div style={{ color: '#1d4ed8', marginBottom: '6px' }}>
                  Rezerva pro slevu při min. marži 50 %
                </div>
        ...
        >
          <h2 style={{ margin: '0 0 16px 0', fontSize: '24px' }}>Položky</h2>

          {itemsError ? (
            <p style={{ color: '#b91c1c' }}>Nepodařilo se načíst položky: {itemsError.message}</p>
          ) : calculationItems.length === 0 ? (
            <p style={{ color: '#6b7280' }}>Kalkulace zatím nemá žádné položky.</p>
          ) : (
            <div style={{ display: 'grid', gap: '20px' }}>
              <div style={customerPanelStyle}>
                ...
                {customerItems.length === 0 ? (
                  <p style={{ margin: 0, color: '#6b7280' }}>Nejsou evidované žádné zákaznické položky.</p>
                ) : (
                    ...
                    {customerItems.map((item) => (
                      ...
                      >
                        ...
                        <div style={{ display: 'grid', gap: '8px' }}>
                          <div><strong>Množství:</strong> {Number(item.quantity ?? 0)}</div>
                          <div><strong>Jednotka:</strong> {item.unit || '-'}</div>
                          <div><strong>Jednotková cena:</strong> {formatCurrency(item.unit_price)}</div>
                          <div><strong>Sazba DPH:</strong> {item.vat_rate != null ? `${Number(item.vat_rate)} %` : '-'}</div>
                          <div><strong>Cena celková:</strong> {formatCurrency(item.total_price)}</div>
                        </div>
                        <div><strong>Poznámka:</strong> {item.note || '-'}</div>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={costPanelStyle}>
                ...
                {costItems.length === 0 ? (
                  <p style={{ margin: 0, color: '#6b7280' }}>Nejsou evidované žádné nákladové položky.</p>
                ) : (
                  <div style={{ display: 'grid', gap: '12px' }}>
                    {costItems.map((item) => (
                      <div
                      ...
                      >
                        ...
                        {item.description && <div style={{ color: '#4b5563' }}>{item.description}</div>}
                        <div style={{ display: 'grid', gap: '8px' }}>
                          <div><strong>Druh nákladu:</strong> {getItemTypeLabel(item.item_type)}</div>
                          <div><strong>Množství:</strong> {Number(item.quantity ?? 0)}</div>
                          <div><strong>Jednotka:</strong> {item.unit || '-'}</div>
                          <div><strong>Náklad/j:</strong> {formatCurrency(item.unit_cost)}</div>
                          <div><strong>Náklad celkový:</strong> {formatCurrency(item.total_cost)}</div>
                        </div>
                        <div>
        ...
        <CalculationDangerZone calculationId={calculation.id} backHref='/kalkulace' />
      </main>
    </DashboardShell>
  )
}
