...
function formatPaymentMethod(value: string | null | undefined) {
  if (value === 'bank_transfer') return 'Prevodem'
  if (value === 'cash') return 'Hotove'
  if (value === 'card') return 'Kartou'
  return value?.trim() || 'Prevodem'
}
...
export function buildInvoicePdf(invoice: InvoicePdfRow, items: InvoicePdfItemRow[]) {
  ...
  addText(contentLines, 62, 688, 8, 'Vystaveno', { color: [0.39, 0.45, 0.55], font: 'bold' })
  ...
  addText(contentLines, 315, 604, 13, 'Odberatel', { font: 'bold' })
  ...
  supplierY -= 18
  addText(contentLines, 44, supplierY, 9, `Ucet: ${bankAccount || '-'}`, { font: 'bold' })
  ...
  addText(contentLines, 342, 156, 10, 'Zaklad')
  ...
  addText(contentLines, 342, 107, 10, 'Celkem k uhrade', { color: [0.39, 0.45, 0.55], font: 'bold' })
  ...
  if (qrPaymentPayload) {
    ...
    addText(contentLines, 60, 137, 8, 'Naskenujte v bankovni aplikaci.', { color: [0.39, 0.45, 0.55] })
    addQrCode(contentLines, qrPaymentPayload, 202, 78, 82)
  } else {
    addRect(contentLines, 44, 92, 252, 86, { fill: [1, 1, 1], stroke: [0.88, 0.9, 0.94] })
    addText(contentLines, 60, 150, 11, 'Platebni udaje', { font: 'bold' })
    addText(contentLines, 60, 130, 9, `Ucet: ${bankAccount || '-'}`)
    addText(contentLines, 60, 113, 9, `VS: ${invoice.variable_symbol ?? '-'}`)
  }

  if (invoice.note) {
    addText(contentLines, 44, 40, 8, `Poznamka: ${invoice.note.replace(/\s+/g, ' ').slice(0, 110)}`, {
